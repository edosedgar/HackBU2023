...
def main():
    ...
    sess = tf.InteractiveSession(graph=tf.Graph(), config=config)

    model = tf.saved_model.loader.load(sess, [tag_constants.SERVING], 'utils/saved_models/stegastamp_pretrained')
